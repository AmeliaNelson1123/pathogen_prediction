...
# booting up s3 bucket
# E2 instance to use: i-05923d7cbe193e08b
s3 = boto3.client('s3')
...
# -------------------------------------------------------------------------
# - AWS/Cloud Computing Integrations --------------------------------------
# -------------------------------------------------------------------------
...
def s3_read_csv(bucket: str, key: str, **kwargs) -> pd.DataFrame:
    # simplest + reliable: download bytes to memory
    obj = s3.get_object(Bucket=bucket, Key=key)
    return pd.read_csv(obj["Body"], **kwargs)

def s3_read_parquet(bucket: str, key: str, **kwargs) -> pd.DataFrame:
    obj = s3.get_object(Bucket=bucket, Key=key)
    return pd.read_parquet(obj["Body"], **kwargs)

def s3_write_csv(df: pd.DataFrame, bucket: str, key: str) -> None:
...
def data_prep(file_info: FileInfo):
    ...
    interval_cols = df.columns[(df.columns != "Salmon_or_camp_test") & (df.columns != "index")]
    ...
    def add_min_max_columns(df, cols):
        ...
        for col in cols:
            ...
            if isinstance(df[col].iloc[0], (int, float)) or df[col].iloc[0] == "*":
                continue

            bounds = df[col].map(interval_to_bounds)
            mins, maxes = zip(*bounds)
            # print(bounds)

            # adding min and max columns
            new_cols[f"min_{col}"] = pd.Series(mins, index=df.index)
            new_cols[f"max_{col}"] = pd.Series(maxes, index=df.index)
...
def get_train_test(
    df, idx_path, y_col="Salmon_or_camp_test", scaling_used=True
):
    """
    ----- inputs -----
    df: pandas dict
                ...
                log_reg_results.append(
                    {
                        ...
                        "y variable used": "Salmon_or_camp_test",
...
def parse_file_info(file_path: str, orig_file: str, idx_file: None | str = None) -> FileInfo:
    ...
    """
    # get path (s3 and linux key)
    s3_path = PurePosixPath(file_path)

    if file_path == orig_file:
        # processing index file to parse
        p = PurePosixPath(idx_file)
...
def main():
    ...
    rows_k = run_files_parallel(files_to_test_k, original_file_to_test, max_workers=2)
    if len(rows_k) != 0:
